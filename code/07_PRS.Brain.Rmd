...
## Make male and female groups #
```{r}
male.eur.prs_filtered <- filter(eur.prs_filtered, sex == "M")
female.eur.prs_filtered <- filter(eur.prs_filtered, sex == "F")
```

## PRS - Whole brain #
```{r}
which(colnames(eur.prs_filtered) == "smri_vol_scs_wholeb")  # 569
which(colnames(eur.prs_filtered) == "mrisdp_453")  # 570
which(colnames(eur.prs_filtered) == "mrisdp_151")  # 571
...
for (j in 1:length(datalist)) {
  ...
  for (i in 569:571) {
    ...
    df <- rbind(df, data.frame(data = names(datalist)[j],
                               ...
                               N = nobs(mod2),
                               beta_PRS = summary(mod2)$coefficients[2, 1],
                               stderr_PRS = summary(mod2)$coefficients[2, 2],
                               P_PRS = p1[2, 8]))
  }
    df$fdr_PRS = p.adjust(df$P_PRS,'fdr')
    print(df)
}

# Association by sex
datalist <- list(female.eur.prs_filtered, male.eur.prs_filtered)
names(datalist) <- c("female.eur.prs_filtered", "male.eur.prs_filtered")
...
for (j in 1:length(datalist)) {
  ...
  for (i in 569:571) {
    this_var = datalist[[j]][, i]
    mod0 <- lmer(as.matrix(this_var) ~ 
                   interview_age + ses_AvgIncomeParentEdu + pc1 + pc2 + pc3 + pc4 + 
                   (1 | mri_info_deviceserialnumber),
                 data = datalist[[j]], REML = FALSE)
    mod2 <- lmer(as.matrix(this_var) ~ PRS.SCZ.CLOZUK.NoMHC_scale +
                   interview_age + ses_AvgIncomeParentEdu + pc1 + pc2 + pc3 + pc4 + 
                   (1 | mri_info_deviceserialnumber),
                 data = datalist[[j]], REML = FALSE)
    p1 <- anova(mod0, mod2)
...
for (j in 1:length(datalist)) {
  ...
  for (i in 2557:2590) {
    ...
    mod0 <- lmer(as.matrix(this_var) ~  sex + 
                   ...
                   (1 | mri_info_deviceserialnumber),
                 data = datalist[[j]], REML = FALSE)
    mod1 <- lmer(as.matrix(this_var) ~ PRS.SCZ.CLOZUK.NoMHC_scale + sex + 
                   interview_age + ses_AvgIncomeParentEdu + pc1 + pc2 + pc3 + pc4 + mrisdp_151 +
                   (1 | mri_info_deviceserialnumber),
                 data = datalist[[j]], REML = FALSE)
    p1 <- anova(mod0, mod1)
    df <- rbind(df, data.frame(data = names(datalist)[j],
                               ID = colnames(datalist[[j]])[i], 
                               N = nobs(mod1),
                               beta = summary(mod1)$coefficients[2, 1],
